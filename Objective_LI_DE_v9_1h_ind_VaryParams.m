function [cost] = Objective_LI_DE_v9_1h_ind_VaryParams(x,param)
...
C_LI =  300;
IC_LI = C_LI * Eb_init; % Assuming a price of $300/kWh
...
P_DE_rated = 7; % Rated maximum power of diesel generator (kW) = approx. peak load * 1.5
...
C_DE = 3.20; % Diesel price (in $/US gal)
...
for t = 1:1:8760 % Simulate over one year with a time-step of 1 h
    ...
    C_fuel_DE = C_fuel_DE + C_DE * (fuel_DE / 3.78541); % Diesel fuel cost assuming a price of $3.20/US liquid gallon (value for east Africa from NREL ReOpt)
...
C_fuel_DE = C_DE * (fuel_DE / 3.78541);
DE_startup = SUC_DE;
DE_shutdown = SDC_DE;
