...
cvx_begin
    % CVX variables can be vectors! 
    variable P_DE(1,24)
    variable P_b_LI(1,24)
    ...
    Cost_base = Fixed_OM_DE + Var_OM_DE + C_fuel_DE;
    ...
    P_DE >= zeros(1,24);
    P_DE <= P_DE_rated*ones(1,24);
    DPSP <= 0.005;
    soc_LI >= SOC_min_LI*ones(1,25);
    soc_LI <= SOC_max_LI*ones(1,25);
    P_b_LI <= P_max_LI*ones(1,24);
    P_b_LI >= -P_max_LI*ones(1,24);
