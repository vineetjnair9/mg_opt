function [cost] = Objective_LA_MT_v2_1h(x)
...
for t = 1:1:8760 % Simulate over one year with a time-step of 1 h
    ...
    elseif (P_RES(t) > P_load/eta_inv) % Excess supply/generation
        ...
        elseif (soc_LA(t) <= SOC_min_LA || Excess_demand > 0) % Need to use backup MT to meet excess load
...
IC = IC_s + IC_w + IC_LA + IC_MT + IC_inv;

% Total annual recurring costs each year
C_rec = OM_s + OM_w + OM_LA + Fixed_OM_MT + Var_OM_MT + C_fuel_MT + MT_startup + MT_shutdown;
