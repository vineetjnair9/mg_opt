function [cost] = Objective_LI_DE_v3_1h(x)
% With modified strategy for excess demand scenario/case
% Assume DE is used only to meet excess load directly and can't be used to charge battery storage
% Also normalized LCOE / emissions
...
t_LI = 15; % Average lifetime of Li-ion battery (y) - Tesla powerwall warranty 
...
% t_DE = 14; % Average lifetime of diesel engine (y) 
...
IC_inv = 2 * P_inv_rated * 1000; % Assuming unit price of $2/W
...
LCOE = TAC/sum(sum(Load)); % Energy cost/Levelized cost of electricity ($/kWh)
