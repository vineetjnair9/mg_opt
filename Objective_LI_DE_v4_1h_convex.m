function [cost] = Objective_LI_DE_v4_1h(x)
% Convexify by removing discrete DE_ON variable to increase speed 
% all variables now continuous
...
DE_hours = 0; % Tracks no. of online operational hours
...
for t = 1:1:8760 % Simulate over one year with a time-step of 1 h
    ...
    elseif (P_RES(t) > P_load/eta_inv) % Excess supply/generation
        ...
        elseif (soc_LI(t) <= SOC_min_LI || Excess_demand > 0) % Need to use backup DE to meet excess load
            ...
            if (t ~= 1 && P_DE(t) == 0) % i.e. DE was offline in the last/most recent time step
                DE_startup = DE_startup + SUC_DE;
                % Ramp_actual = (Excess_Demand - P_DE(t-1))/60 % actual ramp rate (W/min)
            end
        else
            %P_DE(t) = 0;
            if (t ~= 1 && P_DE(t) > 0) % i.e. DE was online in the last/most recent time step
...
Var_OM_DE = Variable_OM_DE * DE_hours;
...
t_DE_rep = (t_DE/DE_hours); % No. of years after which DE needs to be replaced
